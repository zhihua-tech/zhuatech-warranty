...
<p align="center"><img src="docs/images/product-overview.svg" alt="知华科技售后与质保管理系统产品概览" width="920"></p>

## 售后权益与索赔履约平台

系统用序列号串联权益、故障、维修、备件和费用，让每笔质保成本有据可查。

ZhuaTech Warranty 是知华科技（**上海如静知华信息科技有限公司**）维护的前后端分离企业应用社区源码版。产品、实施与技术服务信息请访问[知华科技官网](https://www.zhuatech.cn/)。
...
购买日期、质保期、凭证、除外责任、误用与赔付上限判定。

- 接口：`POST /api/enterprise/warranty/check-coverage`
...
- 预留 CRM、ERP、WMS、服务商与客户门户 的适配器边界，不包含任何真实密钥
...
Java 工程包：`cn.zhuatech.warranty`。前端默认端口：`8309`。
...
新增维修、换货或赔付前的企业级决策门禁，统一核对质保权益、除外责任、反欺诈、购买凭证、序列号、诊断、估价、库存、客户确认与职责分离。详见[企业质保索赔决策说明](docs/ENTERPRISE_CLAIM_DECISION.md)。
...
SEO：质保管理系统、售后管理、Warranty、索赔管理、维修工单、知华科技、上海软件开发、企业信息化、软件项目外包。
